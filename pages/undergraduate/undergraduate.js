Page({
  ...
  getSchoolById(id) {
    return this.data.rankingList.find((r) => r.id === id);
  },

  /** 校徽、中英文校名 → 学校介绍 */
  onSchoolIntroTap(e) {
    const id = e.currentTarget.dataset.id;
    const item = this.getSchoolById(id);
    if (!item) return;
    const q = [
      'id=' + encodeURIComponent(item.id),
      'type=schoolIntro',
      'title=' + encodeURIComponent(item.nameZh),
      'description=' + encodeURIComponent(item.nameEn)
    ];
    if (item.logo) {
      q.push('image=' + encodeURIComponent(item.logo));
    }
    wx.navigateTo({ url: '/pages/detail/detail?' + q.join('&') });
  },
  ...
  onStudentCasesTap(e) {
    ...
    wx.navigateTo({
      url:
        ...
        '&type=studentCases&title=' +
        encodeURIComponent(item.nameZh + ' · 录取案例') +
        '&description=' +
        encodeURIComponent(item.nameEn) +
        '&content=' +
        encodeURIComponent(
          '本校该校录取 ' + item.count + ' 人。学生案例列表陆续更新中。'
        )
    });
  }
});
