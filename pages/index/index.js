...
Page({
  ...
  // 升学榜单入口：本科榜 / 本科转学榜 / 艺术榜
  onRankEntryTap: function(e) {
    const id = e.currentTarget.dataset.id;
    if (id === 'undergraduate') {
      wx.navigateTo({ url: '/pages/undergraduate/undergraduate' });
      return;
    }
    if (id === 'transfer') {
      wx.showToast({ title: '本科转学榜即将上线', icon: 'none' });
      return;
    }
    if (id === 'art') {
      wx.showToast({ title: '艺术榜即将上线', icon: 'none' });
    }
  },
  ...
  checkLoginStatus: function() {
    const { isLoggedIn, userInfo, isAdmin } = userUtil.checkLoginStatus();
    this.setData({
      isLoggedIn,
      isAdmin,
